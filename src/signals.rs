...
impl<'py> DeferedSignal<'py> {
    ...
    pub fn new(py: Python<'py>, signal_name: &str) -> Result<Self> {
        ...
        set_sig.call1((signal, signal_mod.getattr("SIG_DFL")?)).map(|_| ())?;
        Ok(DeferedSignal{set_sig, signal, default_handler, signal_name: signal_name.to_string()})
    }
} 
...
impl<'py> Drop for DeferedSignal<'py> {
    ...
    fn drop(&mut self) {
        self.set_sig
            ...
            .map(|_| ()).expect("Unable to restore default SIGINT handler.");
    }
}
